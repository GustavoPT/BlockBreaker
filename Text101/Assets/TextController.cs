...
public class TextController : MonoBehaviour {
	...
	void stairs_0 (){
			text.text = "You start walking up the stairs towards the outside light." +
						"You realise it's not break time, and you'll be caught immediantely. " +
	...
	void sheets_0(){
			text.text = "You are still in your cell, You can't believe you sleep in these things. Surely it's  " +
	...
	void mirror(){
			text.text = "You are still in your cell, You are now looking at the mirror in which you see your own death!"+
	...
	void lock_0(){
			text.text = "You are still in your cell, You are now checking the lock which unfortunately....is locked\n\n"+
						"Press R to Return to roaming your cell";
		if(Input.GetKeyDown(KeyCode.R)){myState = States.cell;}
	}
	void sheets_1(){
			text.text = "You are still in your cell, You are in sheets still dirty why you looking here?nimrow\n\n" +
						"Press R to Return to roaming your cell with the mirror"; 
		if (Input.GetKeyDown(KeyCode.R)){myState = States.cell_mirror;}
	}
	void cell_mirror(){
		text.text = "You have taken the mirror you bastard and found a key!\n\n"+
					"Press R to put the mirror back, L to check the Lock, S to Check sheets";
		if(Input.GetKeyDown(KeyCode.R)){myState = States.cell;}
		else if(Input.GetKeyDown(KeyCode.S)){myState = States.sheets_1;}
		else if(Input.GetKeyDown(KeyCode.L)){myState = States.lock_1;}
	}
	void lock_1(){
		text.text = "You are now looking at the lock\n\n"+
					"Press R to Return to roaming your cell with the mirror,O to Open the lock";
		if(Input.GetKeyDown(KeyCode.R)){myState = States.cell_mirror;}
		else if(Input.GetKeyDown(KeyCode.O)){myState = States.corridor_0;}
	}
	void corridor_0(){	
			text.text = "You have entered corridor 0!\n"+
